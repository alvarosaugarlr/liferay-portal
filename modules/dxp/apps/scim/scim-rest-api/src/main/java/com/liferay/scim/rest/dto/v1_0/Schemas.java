...
public class Schemas implements Serializable {
	...
	public static Schemas toDTO(String json) {
		return ObjectMapperUtil.readValue(Schemas.class, json);
	}

	public static Schemas unsafeToDTO(String json) {
		return ObjectMapperUtil.unsafeReadValue(Schemas.class, json);
	}
	...
	public SchemaDefinition[] getSchemaDefinitions() {
		if (_schemaDefinitionsSupplier != null) {
			schemaDefinitions = _schemaDefinitionsSupplier.get();

			_schemaDefinitionsSupplier = null;
		}
	...
	public void setSchemaDefinitions(
		...
		_schemaDefinitionsSupplier = () -> {
			try {
				return schemaDefinitionsUnsafeSupplier.get();
			}
			catch (RuntimeException runtimeException) {
				throw runtimeException;
			}
	...
	@JsonIgnore
	private Supplier<SchemaDefinition[]> _schemaDefinitionsSupplier;
	...
	public String toString() {
		...
		SchemaDefinition[] schemaDefinitions = getSchemaDefinitions();
		...
		if (schemaDefinitions != null) {
			...
			for (int i = 0; i < schemaDefinitions.length; i++) {
				sb.append(String.valueOf(schemaDefinitions[i]));

				if ((i + 1) < schemaDefinitions.length) {
					sb.append(", ");
				}
			}
	...
	@Schema(
		...
		defaultValue = "com.liferay.scim.rest.dto.v1_0.Schemas",
		name = "x-class-name"
	)
